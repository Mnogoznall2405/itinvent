# 📖 Основное руководство по IT-invent Bot

Telegram-бот для управления инвентаризацией IT-оборудования с автоматическим распознаванием серийных номеров и генерацией актов приема-передачи.

## 🚀 Быстрый старт

### Установка

```bash
# 1. Установите зависимости
pip install -r requirements.txt

# 2. Настройте .env файл (скопируйте .env.example)
# Укажите токен бота, настройки БД и API ключи

# 3. Запустите бота
python -m bot.main
```

### Первый запуск

После установки зависимостей и настройки `.env`:

```bash
python -m bot.main
```

Бот готов к работе! 🎉

---

## ✨ Основные возможности

- 🔎 **Поиск оборудования** - по серийному номеру (текст/фото) или сотруднику
- 🤖 **OCR** - автоматическое распознавание серийных номеров с фотографий
- 💡 **Автоподсказки** - умный поиск сотрудников при вводе ФИО
- 📦 **Перемещение с актом** - генерация PDF-актов приема-передачи
- 🗄️ **Множественные БД** - работа с несколькими базами данных
- 📊 **Экспорт данных** - выгрузка в Excel/CSV с отправкой на email, экспорт баз данных
- 🔧 **Замена комплектующих МФУ** - учет замен картриджей, фьюзеров, фотобарабанов с автоматическим определением совместимых моделей
- 🔋 **Замена батареи ИБП** - полный цикл учёта замены батарей с OCR-распознаванием
- 📦 **Установка оборудования** - учёт установки нового оборудования
- 🧠 **Умная детекция** - LLM-анализ для определения типа принтера и доступных комплектующих
- 📝 **Регистрация работ** - выполненные работы по оборудованию
- 🧾 **Ненайденное оборудование** - учёт оборудования вне БД

---

## 🎯 Основные команды

- `/start` - Главное меню
- `/help` - Справка по командам
- `/cancel` - Отмена текущей операции
- `/done` - Завершение загрузки фотографий

---

## 📋 Требования

- Python 3.8+
- Microsoft SQL Server (доступ через ODBC)
- Telegram Bot Token
- OpenRouter API Key (для OCR)
- Microsoft Word (опционально, для конвертации PDF)

---

## 🤝 Поддержка

При возникновении проблем:

1. Проверьте логи: `type bot.log`
2. Убедитесь, что все зависимости установлены
3. Проверьте настройки в `.env`
4. Обратитесь к технической документации

---

**Версия:** 2.1.0
**Последнее обновление:** 2026-01-27